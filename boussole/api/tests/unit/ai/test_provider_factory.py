...
from tests.unit.ai.conftest import settings
...
class TestSelectionParConfiguration:
    ...
    def test_sans_cle_le_provider_reel_n_est_pas_active(self) -> None:
        # Dégradation explicite (D18) : jamais d'appel réseau sans clé, et
    ...
    def test_un_provider_inconnu_est_ignore_au_lieu_de_casser_l_application(self) -> None:
        provider = get_llm_provider("extract_job", settings(ai_provider="mistral-maison"))
    ...
    def test_le_fallback_configure_est_ajoute_comme_second_candidat(self) -> None:
        ...
        assert [name for name, _ in provider.candidates] == ["anthropic", "fake"]
