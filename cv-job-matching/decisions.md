...
## D53 — La langue est une affaire d'interface, jamais de l'API ✅ *(finitions produit)*
- **Décision** : le front construit ses libellés à partir des **codes** rendus par l'API (`dimension`, `reason`, `code` de bloquant). Le `label` de l'API n'est plus qu'un dernier recours, pour un code que le front ne connaît pas encore. Six traductions de critères bloquants ajoutées en fr et en.
- **Justification** : l'API rend ses libellés **en français, sans condition** — elle ne lit jamais `Accept-Language`, alors que le proxy BFF le lui transmet. La précédence était `if (unknown.label) return unknown.label`, si bien qu'un utilisateur en anglais lisait « Role similarity » (front) juste au-dessus de « Similarité du métier : comparaison indisponible (outil non calibré) » (API) — sur l'écran même censé lui expliquer son score. Les traductions `unknown.*Fallback`, présentes et correctes en `en`, étaient du **code mort**.
- **Alternatives** : faire lire `Accept-Language` à l'API (rejeté au MVP : cela déplace la table de traduction côté serveur et impose de la maintenir en double avec celle du front, qui existe déjà et est vérifiée par le contrôle de parité) ; ne rendre que des codes (rejeté : casse la compatibilité et prive un client tiers de tout libellé).
- **Bénéfice non recherché** : les libellés de bloquants de l'API sont les descriptions de la **règle**, écrites pour la spécification (« Langue requise absente ou ≥ 2 niveaux CECRL en dessous »). Le front dit maintenant ce qui concerne la personne.
- **Corrigé au passage** : la clé i18n `fine_preferences` ne correspondait à aucune dimension — l'identifiant réel est `preference_boost`. Le test d'appartenance échouait et l'utilisateur lisait `preference boost`, le repli dé-snake-casé, en français comme en anglais.
- **Vérifié par** : `tests/unknown-label.test.ts` (dont un test qui disait l'inverse et **encodait le défaut**, retourné avec sa justification).

## D54 — Un seuil qui décide de ce que l'utilisateur lit vient de l'endroit qui le définit ✅ *(finitions produit)*
- **Décision** : `MatchResultOut` porte `explanation_thresholds` (obligatoire). Le front les consomme au lieu de les recopier.
- **Justification** : trois constantes en dur dans `match-panel.tsx`, dupliquées de `scoring-config.json`. Elles coïncidaient — jusqu'à la première recalibration, après quoi l'écran aurait classé forces et lacunes selon d'anciennes valeurs, **sans que rien ne le signale**. Le champ est rendu obligatoire délibérément : optionnel, il aurait laissé le front garder ses constantes en repli, donc le défaut intact pour qui ne lit pas la note.
- **Vérifié par** : `tests/unit/test_finitions_produit.py`.

## D55 — Le quota d'explication existe, après le cache et sans emporter la génération ✅ *(finitions produit)*
- **Décision** : 10/h et 40/j sur `POST /jobs/{id}/explanation`, prélevés **après** la lecture du cache, sur des compteurs **séparés** de ceux de la génération de documents. Redis injoignable ⇒ on laisse passer (D18).
- **Justification** : la route n'avait **aucun** quota, alors que 12 §11 en impose un. Vérifié : huit appels consécutifs sur huit offres différentes (donc hors cache), zéro 429. Un utilisateur qui parcourt deux cents offres déclenchait deux cents appels LLM non plafonnés — un coût direct, et une porte ouverte pour qui le fait exprès. Le front portait pourtant depuis toujours une branche 429 dédiée qui ne pouvait jamais s'afficher.
- **Trois choix, trois raisons** : *après le cache*, parce qu'une explication déjà calculée ne coûte aucun appel au fournisseur et que faire payer un jeton à qui relit une offre serait absurde ; *compteurs séparés*, parce qu'expliquer un score ne doit pas empêcher d'écrire une lettre — un utilisateur ne comprendrait pas ; *fail-open*, par cohérence avec le limiteur global, un quota indisponible ne devant pas suspendre une fonction.
- **Vérifié par** : `tests/unit/test_finitions_produit.py`, dont un test qui contrôle l'**ordre** cache/quota dans le code.

## D56 — L'onboarding lit la base ✅ *(finitions produit)*
- **Décision** : `GET /me` calcule les trois indicateurs par une requête à trois `EXISTS` (CV importé, profil **validé**, préférences définies).
- **Justification** : c'étaient trois littéraux `False`, avec pour commentaire « M1 : tant que les modules profiles/preferences (M2+) ne fournissent pas leurs services ». Ils les fournissent depuis quatre jalons. La carte « Mon CV » du tableau de bord affichait donc à vie « Aucun CV importé pour le moment » et proposait « Importer mon CV » plutôt que « Réimporter » — quel que soit le nombre de CV réellement importés. Le composant note pourtant qu'il lit `GET /me`, « jamais d'état local seul » : **la source de vérité choisie était celle qui mentait**.
- **Pourquoi aucun test ne le voyait** : la doublure en mémoire rendait elle aussi trois `False` en dur. Quand le fake dit la même chose que le code, aucun test ne peut voir que le code ne regarde rien. La doublure reflète désormais ce que le test a posé.
- **Détail qui compte** : un profil `draft` ne compte **pas** comme validé. Le dire validé enverrait l'utilisateur vers un écran de matching qui répond 409.
- **Vérifié par** : `tests/unit/test_finitions_produit.py` et `tests/integration/test_onboarding_state.py` — la requête est en SQL brut sur `cv_documents`, table sans modèle ORM : une faute de nom de colonne ne se voit que contre une vraie base. Elle s'y est effectivement vue (`storage_key` au lieu de `file_key`).

## D57 — Le contrat publié décrit exactement ce qui est servi ✅ *(finitions produit)*
- **Décision** : les **quatorze** routes servies et absentes d'`openapi.yaml` y sont ajoutées, avec leurs schémas. Un test compare les deux ensembles dans les deux sens.
- **Justification** : manquaient toute l'édition de profil (compétences, formations, langues), le détail/la modification/la suppression d'une candidature, le report d'extraction de CV et le téléchargement de l'archive d'export RGPD — toutes appelées par le front en production. Un contrat incomplet n'est pas une documentation en retard : c'est la seule référence de qui intègre l'API sans lire le code, et depuis D48 le seul document lisible, le schéma généré étant fermé dès staging.
- **Les deux sens** : une route promise et jamais servie fait écrire du code d'intégration contre du vide. Le test refuse les deux.
- **Vérifié par** : `tests/unit/test_contrat_openapi.py`, dont l'efficacité a été contrôlée en retirant une route du contrat (échec attendu, message nommant la route).
