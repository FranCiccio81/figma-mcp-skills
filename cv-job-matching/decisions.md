...
- **Vérifié par** : `tests/unit/applications/test_idempotency_shared.py` (deux services, un magasin partagé), `tests/unit/core/test_locks.py`.
...
- **Point de vigilance** : ce produit manipule des CV. Le nettoyage des événements (corps, cookies, en-têtes d'identité, variables locales des piles) est une **fonction pure testée sans le paquet installé** — faire dépendre un contrôle de confidentialité d'une dépendance optionnelle reviendrait à ne pas le tester en CI.
- **Vérifié par** : `tests/unit/core/test_observability.py`, `tests/unit/privacy/test_deletion_email.py`, et un envoi réel contre un serveur SMTP.
