...
| **N4** | **Rétention 13 mois d'`ai_calls` non implémentée.** La purge RGPD **par utilisateur** l'est (anonymisation `user_id → NULL`), mais la purge **par âge** est un TODO : aucune tâche beat ne supprime les lignes de plus de 13 mois. Engagement de 11 §3 non tenu. | À implémenter en tâche `maintenance.*` planifiée. | Privacy Eng + Backend | Avant lancement public |
