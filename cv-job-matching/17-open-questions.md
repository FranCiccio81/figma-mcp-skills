...
| **N13** | **Un poste HYBRIDE n'est pas rédhibitoire pour un candidat exigeant le full remote.** La spec pose `requis × hybride = 0,4` sans bloquant (06 §, table de télétravail) ; seul `requis × sur-site` bloque. Le moteur l'applique fidèlement. Mais un candidat dont le full remote est une **contrainte** — et non une préférence, le vocabulaire distingue les deux — ne peut pas plus tenir un poste hybride qu'un poste sur site : il devra être présent certains jours. Il reçoit donc ces offres sans badge, et peut construire une candidature autour d'un poste qu'il ne peut pas accepter. Découvert en construisant le jeu d'évaluation, où six annotations « bloquant attendu » contredisaient la spec. | 🟡 Deux issues cohérentes, à trancher : (a) `requis × hybride` devient bloquant, et « préféré » couvre le cas négociable ; (b) la spec est confirmée, et l'interface doit alors dire clairement que « requis » signifie « fortement souhaité ». Le statu quo — un niveau nommé « requis » qui ne bloque pas — est le seul choix indéfendable. | CPO + Lead Product Designer | Avant alpha |
| **N14** | **`title_similarity` est inerte ET compte comme un fait connu.** Mesuré sur les 36 paires du jeu d'évaluation : sous-score **0,00 partout**, statut **« connu »**. La dimension pèse 15 % — la troisième plus lourde — et ne discrimine rien : elle abaisse uniformément tous les scores. Cause : le provider par défaut est lexical (D27) alors que les seuils `zero_below=0,55` / `one_above=0,80` ont été posés pour des vecteurs sémantiques. **Le plus gênant est l'indice de confiance** : une dimension qui ne mesure rien est publiée comme connue, donc la confiance affichée est surévaluée de 15 points de poids. C'est la première mesure chiffrée de l'impact de N1. | 🟡 Court terme : rendre la dimension **inconnue** (k=0) quand le provider est lexical — la renormalisation la retirerait alors du calcul et de la confiance, ce qui est honnête. Long terme : Q11 puis recalibration (Q12/Q41). | ML Eng + Architect | Court terme **avant alpha** |
| **N15** | **La couverture des compétences requises récompense les offres peu exigeantes.** `skills_required` (25 %, méthode `coverage`) donne 1,00 à une offre qui n'exige qu'une compétence que le candidat possède. Mesuré : « Développeur Python Junior » (une exigence : `python`) obtient 1,00 et un score global de 60 pour un profil **DevOps senior exigeant le full remote**, à qui ce poste ne convient pas — pendant qu'une offre pertinente listant cinq exigences dont quatre satisfaites plafonne à 0,80. Effet net : Spearman tombe à **0,570** sur ce profil, **sous la porte de 0,60**. Comme les offres réelles varient beaucoup en précision de rédaction, le biais favorise systématiquement les annonces vagues. | 🟡 Pistes non arbitrées : pondérer la couverture par le nombre d'exigences, plancher sur le nombre d'exigences en deçà duquel la dimension devient peu fiable, ou prise en compte de la spécificité de la compétence. Aucune ne doit être choisie sans mesure — c'est précisément à cela que sert le harnais. | ML Eng + CPO | Avant alpha |
